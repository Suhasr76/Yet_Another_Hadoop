...
def Pass_To_Mapper(path, offset,block_size):
...
# print(Matrix[0][0]);
    for i in range(number_of_blocks*replication_factor):
        ...
        if(int(Matrix[i][0]) in Accessed_Blocks):
            ...
            input.append(Pass_To_Mapper(Matrix[i][1], Matrix[i][2],block_size))
...
#yah('input/crypto.txt','output/output1.txt','config/test_config.json','mapper','reducer')
